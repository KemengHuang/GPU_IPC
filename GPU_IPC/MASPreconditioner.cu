...
__global__
void _buildCML0(const unsigned int* _neighborStart, unsigned int* _neighborNum, unsigned int* _neighborList, unsigned int* _fineConnectedMsk, int vertNum) {
...
__device__
unsigned int _LanemaskLt(int laneIdx) {
	return (1U << laneIdx) - 1;
}

__global__
void _preparePrefixSumL0(int* _prefixOriginal, unsigned int* _fineConnectedMsk, int vertNum) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= vertNum) return;
	int warpId = idx / 32;
	...
	while (connectMsk != -1) {
		...
		visited |= (1U << nextVist);
		connectMsk |= cacheMask[nextVist + localWarpId * 32];//__shfl(cacheMask, nextVist);//?????!!!!!
	}
...
__global__
void _buildLevel1(int2* _levelSize, int* _coarseSpaceTable, int* _goingNext, const unsigned int* _fineConnectedMsk, const int* _prefixSumOriginal, const int* _prefixOriginal, int vertNum) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= vertNum) return;
	int warpId = idx / 32;
	int localWarpId = threadIdx.x / 32;
	...
	if (idx == vertNum - 1) {
		...
		_levelSize[1].y = (vertNum + 31) / 32 * 32;
	}
	...
	unsigned int theLanePrefix = __shfl(lanePrefix, elected_lane);
...
__global__
void _buildConnectMaskLx(const unsigned int* _neighborStart, unsigned int* _neighborNum, unsigned int* _neighborList, int* _coarseSpaceTable, unsigned int* _nextConnectedMsk, const unsigned int* _fineConnectedMsk, int level, int vertNum) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= vertNum) return;
	int warpId = idx / 32;
	int localWarpId = threadIdx.x / 32;
...
__global__
void _nextLevelCluster(unsigned int* _nextConnectedMsk, unsigned int* _nextPrefix, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	while (true) {
		...
		connMsk |= cachedMsk[nextVisit + localWarpId * 32];//__shfl(cachedMsk, nextVisit);
...
__global__
void _prefixSumLx(int2* _levelSize, unsigned int* _nextPrefix, unsigned int* _nextPrefixSum, unsigned int* _nextConnectMsk, int* _goingNext, int level, int levelBegin, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
	int warpId = idx / 32;
	int localWarpId = threadIdx.x / 32;
	...
	if (idx == number - 1) {
		...
		_levelSize[level + 1].y = levelBegin + (number + 31) / 32 * 32;
	}
...
__global__
void _computeNextLevel(int* _coarseSpaceTable, unsigned int* _nextConnectMsk, int level, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
...
__global__
void _aggregationKernel(int* _denseLevel, int4* _coarseTable, int* _goingNext, int levelNum, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
...
__global__ void _prepareHessian(const __GEIGEN__::Matrix12x12d* Hessians12, const __GEIGEN__::Matrix9x9d* Hessians9,
	...
	__GEIGEN__::Matrix96x96MT* P96, int numbers4, int numbers3, int numbers2, int numbers1, int* _goingNext, int levelNum) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (idx < numbers4) {
		...
		int roffset = qrid % 3;
		int coffset = qcid % 3;
		Precision_TM Hval = Hessians12[Hid].m[qrid][qcid];
		...
		atomicAdd(&(P96[cPid].m[(vertRid % 32) * 3 + roffset][(vertCid % 32) * 3 + coffset]), Hval);
		...
		Precision_TM Hval = Hessians9[Hid].m[qrid][qcid];
		...
		Precision_TM Hval = Hessians6[Hid].m[qrid][qcid];
		...
		Precision_TM Hval = Hessians3[Hid].m[qrid][qcid];
...
__global__
void __setMassMat_P96(const double* _masses, const int* _goingNext, __GEIGEN__::Matrix96x96MT* _Mat96, int levelNum, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
	int warpId = idx / 32;
	int laneId = idx % 32;

	Precision_TM mass = _masses[idx];
...
//__global__ void __inverse_P96x96(__GEIGEN__::Matrix96x96d* P96, __GEIGEN__::Matrix96x96* invP96, int numbers) {
//	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void __inverse3_P96x96(__GEIGEN__::Matrix96x96MT* P96, __GEIGEN__::Matrix96x96T* invP96, int numbers) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numbers) return;
	...
	__threadfence();
	int j = 0;
	Precision_TM rt = P96[matId].m[0][0];
	__syncthreads();
	__threadfence();
	while (/*loopId[localMatId]*/j < 96) {
		if (i <= j)
		...
		for (int k = 0; k < 96; k++) {
			if (k != j) {
				Precision_TM rate = -P96[matId].m[k][j];
				__syncthreads();
				__threadfence();
				if (i <= j)
					invP96[matId].m[k][i] += rate * invP96[matId].m[j][i];
...
__global__ void __warp_inverse_P96x96(__GEIGEN__::Matrix96x96MT* P96, __GEIGEN__::Matrix96x96T* invP96, int numbers) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numbers) return;
	...
	int j = 0;
	Precision_TM rt = P96[matId].m[j][j];
	while (j < 96) {
...
__global__ void __buildMultiLevelR_optimized(const double3* _R, Precision_T3* _multiLR, int* _goingNext, unsigned int* _fineConnectMsk, int levelNum, int numbers) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numbers) return;
	...
	__shared__ Precision_T c_sumResidual[DEFAULT_BLOCKSIZE * 3];
	...
	if (connectMsk == -1) {
		...
		atomicAdd(c_sumResidual + localWarpId * 32 + elected_lane + 2 * DEFAULT_BLOCKSIZE, r.z);
		...
		if (electedPrefix == 0) {
			while (level < levelNum - 1) {
				...
				atomicAdd((&((_multiLR + idx)->x) + 1), c_sumResidual[threadIdx.x + DEFAULT_BLOCKSIZE]);
				atomicAdd((&((_multiLR + idx)->x) + 2), c_sumResidual[threadIdx.x + DEFAULT_BLOCKSIZE * 2]);
			}
		}
	}
}

__global__ void __buildMultiLevelR(const double3* _R, Precision_T3* _multiLR, int* _goingNext, int levelNum, int numbers) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numbers) return;
...
__global__ void __collectFinalZ(double3* _Z, const Precision_T3* d_multiLevelZ, const int4* _coarseTable, int levelnum, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
...
__global__ void _schwarzLocalXSym0(const __GEIGEN__::Matrix96x96T* P96, const Precision_T3* mR, Precision_T3* mZ, int number) {
...
__global__ void _schwarzLocalXSym(const __GEIGEN__::Matrix96x96T* P96, const Precision_T3* mR, Precision_T3* mZ, int number) {
	...
	Precision_T rdata = P96[Hid].m[MRid][MCid] * (*(&(mR[vcid].x) + axisId));
...
__global__
void _buildCollisionConnection(unsigned int* _pConnect, const int* _pCoarseSpaceTable, const const int4* _collisionPair, int level, int vertNum, int number) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= number) return;
...
void MASPreconditioner::BuildConnectMaskL0() {
	...
	_buildCML0 << <numBlocks, blockSize >> > (d_neighborStart, d_neighborNum, d_neighborList, d_fineConnectMask, number);
}
...
void MASPreconditioner::BuildLevel1() {
	...
	thrust::exclusive_scan(thrust::device_ptr<int>(d_prefixOriginal), thrust::device_ptr<int>(d_prefixOriginal) + warpNum, thrust::device_ptr<int>(d_prefixSumOriginal));
	_buildLevel1 << <numBlocks, blockSize >> > (d_levelSize, d_coarseSpaceTables, d_goingNext, d_fineConnectMask, d_prefixSumOriginal, d_prefixOriginal, number);
}
...
void MASPreconditioner::BuildConnectMaskLx(int level) {
	...
	int blockSize = DEFAULT_BLOCKSIZE;
	int numBlocks = (number + blockSize - 1) / blockSize;
	_buildConnectMaskLx << <numBlocks, blockSize >> > (d_neighborStart, d_neighborNum, d_neighborList, d_coarseSpaceTables, d_nextConnectMask, d_fineConnectMask, level, number);
}
...
void MASPreconditioner::ComputeNextLevel(int level) {
	...
	int blockSize = DEFAULT_BLOCKSIZE;
	int numBlocks = (number + blockSize - 1) / blockSize;
	_computeNextLevel << <numBlocks, blockSize >> > (d_coarseSpaceTables, d_nextConnectMask, level, number);
}
...
void MASPreconditioner::PrefixSumLx(int level) {
	...
	int warpNum = (number + 31) / 32;
	thrust::exclusive_scan(thrust::device_ptr<unsigned int>(d_nextPrefix), thrust::device_ptr<unsigned int>(d_nextPrefix) + warpNum, thrust::device_ptr<unsigned int>(d_nextPrefixSum));

	_prefixSumLx << <numBlocks, blockSize >> > (d_levelSize, d_nextPrefix, d_nextPrefixSum, d_nextConnectMask, d_goingNext, level, levelBegin, number);
}
...
void MASPreconditioner::AggregationKernel() {
	...
	int blockSize = DEFAULT_BLOCKSIZE;
	int numBlocks = (number + blockSize - 1) / blockSize;
	_aggregationKernel << <numBlocks, blockSize >> > (d_denseLevel, d_coarseTable, d_goingNext, levelnum, number);
}
...
void MASPreconditioner::BuildCollisionConnection(unsigned int* connectionMsk, int* coarseTableSpace, int level, int cpNum) {
	int number = cpNum;
	int blockSize = DEFAULT_BLOCKSIZE;
	int numBlocks = (number + blockSize - 1) / blockSize;

	_buildCollisionConnection << <numBlocks, blockSize >> > (connectionMsk, coarseTableSpace, _collisonPairs, level, totalNodes, number);
}
...
void MASPreconditioner::PrepareHessian(const BHessian& BH, const double* masses) {
	...
	_prepareHessian << <numBlocks, blockSize >> > (BH.H12x12, BH.H9x9, BH.H6x6, BH.H3x3, BH.D4Index, BH.D3Index, BH.D2Index, BH.D1Index, d_Mat96, BH.DNum[3] * 144, BH.DNum[2] * 81, BH.DNum[1] * 36, BH.DNum[0] * 9, d_goingNext, levelnum);
	...
	number *= 96;
	numBlocks = (number + blockSize - 1) / blockSize;
	__inverse3_P96x96 << <numBlocks, blockSize >> > (d_Mat96, d_inverseMat96, number);

	cudaEventRecord(end2);


	//__GEIGEN__::Matrix96x96d h_mat96;
	//CUDA_SAFE_CALL(cudaMemcpy(&h_mat96, d_inverseMat96 +1, sizeof(__GEIGEN__::Matrix96x96d), cudaMemcpyDeviceToHost));

	//ofstream out("inverse1.txt");
...
void MASPreconditioner::BuildMultiLevelR(const double3* R) {
	...
	__buildMultiLevelR_optimized << <numBlocks, blockSize >> > (R, d_multiLevelR, d_goingNext, d_fineConnectMask, levelnum, number);
...
void MASPreconditioner::SchwarzLocalXSym() {
	...
	int number = matNum * 96 * 32;
	int blockSize = 96 * 3;
	int numBlocks = (number + blockSize - 1) / blockSize;

	_schwarzLocalXSym0 << <numBlocks, blockSize >> > (d_inverseMat96, d_multiLevelR, d_multiLevelZ, number);
}
...
void MASPreconditioner::setPreconditioner(const BHessian& BH, const double* masses, int cpNum) {
	...
	CUDA_SAFE_CALL(cudaMemcpy(d_neighborList, d_neighborListInit, neighborListSize * sizeof(unsigned int), cudaMemcpyDeviceToDevice));
	...
	CUDA_SAFE_CALL(cudaMemcpy(d_neighborNum, d_neighborNumInit, totalNodes * sizeof(unsigned int), cudaMemcpyDeviceToDevice));
	...
	CUDA_SAFE_CALL(cudaMemset(d_Mat96, 0, totalNumberClusters / BANKSIZE * sizeof(__GEIGEN__::Matrix96x96MT)));
...
void MASPreconditioner::preconditioning(const double3* R, double3* Z) {
	CUDA_SAFE_CALL(cudaMemset(d_multiLevelR + totalNodes, 0, (totalNumberClusters - totalNodes) * sizeof(Precision_T3)));
...
void MASPreconditioner::initPreconditioner(int vertNum, int totalNeighborNum, int4* m_collisonPairs) {
	...
	int totalCluster = ReorderRealtime(0);

	CUDA_SAFE_CALL(cudaMalloc((void**)&d_Mat96, totalCluster / BANKSIZE * sizeof(__GEIGEN__::Matrix96x96MT)));
	CUDA_SAFE_CALL(cudaMalloc((void**)&d_inverseMat96, totalCluster / BANKSIZE * sizeof(__GEIGEN__::Matrix96x96T)));
